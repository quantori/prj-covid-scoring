...
def main(args):
    img_paths, ann_paths, dataset_names = read_supervisely_project(
        sly_project_dir=args.dataset_dir,
    ...
    if set(args.covid_datasets) & set(args.normal_datasets):
        ...
        print(
            '\033[91m' + 'There are common elements in both dataset lists '
    ...
    for subset in subsets:
        ...
        for idx in tqdm(
            range(len(img_paths)),
            desc='Processing of {:s} dataset'.format(subset),
            unit=' images',
        ):
            ...
            img_output = os.path.join(
                args.output_dir, subset, dataset_name, 'img', filename
            )
            mask_output = os.path.join(
                args.output_dir, subset, dataset_name, 'mask', filename
            )
            ...
            extracted_values = extract_ann_score(
                dataset_name, args.normal_datasets, ann_path
            )
...
if __name__ == '__main__':
    ...
    if 'covid' in args.dataset_dir:
        ...
        raise ValueError(
            'There is no class name for dataset {:s}'.format(args.dataset_dir)
        )
