...
def run_scoring_training(model_type, covid_scoring_metadata_path, num_classes, epochs, batch_size, dropout,
                         source_ds_dir, output_model_path):
    ...
    data_dist = {'train': {'split_start': 0, 'split_end': 0.8, 'batch_size': batch_size, 'dataset': None},
                 'validation': {'split_start': 0.8, 'split_end': 0.9, 'batch_size': batch_size, 'dataset': None},
    ...
    valid_data = loaded_data['validation']['dataset']

    scoring_models = ScoringModel(model_type=model_type, num_classes=num_classes, dropout=dropout)
    train_metrics, validation_metrics = scoring_models.train_scoring_model(train_data, valid_data, epochs, output_model_path)
...
if __name__ == '__main__':
    ...
    run_scoring_training(args.model_type, covid_scoring_metadata_path, args.num_classes, args.epochs, args.batch_size,
                         args.dropout, args.source_ds_dir, output_model_path)
